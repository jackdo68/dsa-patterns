# Context Manager (Bounded Buffer with Pinned Head)
...
- [Fixed-size sliding window (Find All Anagrams)](fixed-size-find-all-anagrams-in-a-string.md) — same "running window over a sequence" instinct
- [LRU Cache](lru-cache.md) — bounded-capacity store with an eviction policy (LRU vs FIFO)
- [Min Stack](min-stack.md) — auxiliary structure pinned alongside a primary structure
