---
title: "Agent Loop (Tool-Calling State Machine)"
...
- **Combine with [Context Manager](/dsa-patterns/context-manager/)** — the message history grows unboundedly; prune old messages when it exceeds the model's context window.
...
- **Bounded iteration** — same guard pattern as [retry-with-backoff](/dsa-patterns/retry-with-backoff/) (`for i in 0..maxRetries`).
- [DFS Cycle Detection](/dsa-patterns/dfs-cycle-detection-course-schedule-ii/) — the "iteration cap" prevents infinite loops, similar to cycle detection in graphs.
